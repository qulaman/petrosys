-- =============================================================================
-- QuarryOps · Миграция 0016 — пороги в настройках + 3 отложенных детектора
-- org_settings (пороги), детекторы: work_no_fuel, short_trip_interval,
-- continuous_driving. detect_anomalies пересоздаётся целиком (9 детекторов).
-- =============================================================================

create table public.org_settings (
  org_id            uuid primary key default public.current_org_id()
                    references public.organizations(id),
  tanker_gap_liters numeric not null default 20,   -- допуск расхождения замера
  no_fuel_days_trips integer not null default 2,   -- дней без заправки: самосвалы
  no_fuel_days_hours integer not null default 3,   -- дней без заправки: остальная техника
  updated_at        timestamptz not null default now()
);
alter table public.org_settings enable row level security;
create policy "staff read org_settings" on public.org_settings for select to authenticated
  using (org_id = public.current_org_id());
create policy "office manage org_settings" on public.org_settings for all to authenticated
...
insert into public.org_settings (org_id) select id from public.organizations
