-- =============================================================================
-- QuarryOps · Миграция 0022 — правки опытной эксплуатации (21.07)
-- 1) Двойной тип учёта: accounting_type = 'both' (часы + рейсы) — машина видна
--    и в Табеле, и в Рейсах; АВР и так суммирует оба вида начислений.
-- 2) vehicles.source — происхождение записи: импорт / вручную / с поля.
--    Существующий парк — 'import'; новые записи (админка) — 'manual' по умолчанию.
-- 3) Детекторы: over_norm и work_no_fuel учитывают 'both'.
-- =============================================================================

alter table public.vehicles drop constraint vehicles_accounting_type_check;
alter table public.vehicles add constraint vehicles_accounting_type_check
  check (accounting_type in ('hours', 'trips', 'both'));

alter table public.vehicles
  add column if not exists source text not null default 'manual'
  check (source in ('import', 'manual', 'field'));
update public.vehicles set source = 'import';
comment on column public.vehicles.source is
  'Происхождение записи: import — загружена скриптом, manual — создана в админке, field — с полевой формы.';

-- -----------------------------------------------------------------------------
-- detect_anomalies v5: копия v4 с учётом accounting_type='both'
--   over_norm: норматив л/ч применяется и к машинам с двойным учётом;
--   work_no_fuel: 'both' считается самосвалом (пороги по сменности).
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- (7) недопущенная техника — машина×месяц
  insert into anomalies (org_id, type, severity, entity_refs, dedup_key)
  ...
  -- (3) расход выше норматива (месяц) — часы и двойной учёт
  insert into anomalies (org_id, type, severity, entity_refs, dedup_key)
  ...
  where v.org_id = p_org_id and v.accounting_type in ('hours', 'both')
    and v.fuel_norm_per_hour is not null and h.hours > 0
    and (l.liters / h.hours) > v.fuel_norm_per_hour
  on conflict (org_id, dedup_key) do nothing;

  -- (2) работа без топлива с учётом сменности; 'both' считается самосвалом
  insert into anomalies (org_id, type, severity, entity_refs, dedup_key)
  ...
  cross join lateral (
    select case
      when v.accounting_type = 'hours' then v_nf_hours
