-- =============================================================================
-- QuarryOps · Миграция 0019 — detect_anomalies v4: слияние двух веток 0017/0018
--
-- 0017 (anomalies_v3) дала: агрегацию «недопущенной техники» по машине×месяцу,
-- ключ 'date' (дата события) и порог в entity_refs всех детекторов.
-- 0018 (no_fuel_shift_aware) дала: пороги «работы без топлива» по сменности,
-- но была написана поверх v2 и откатила изменения 0017 в функции.
-- v4 объединяет обе ветки и повторяет разовую чистку по-сменных записей.
-- =============================================================================
...
begin
  ...
  -- (7) НЕДОПУЩЕННАЯ ТЕХНИКА — одна аномалия на машину×месяц (из 0017).
  ...
  -- (2) РАБОТА БЕЗ ТОПЛИВА с учётом сменности (из 0018) + ключ 'date':
...
-- -----------------------------------------------------------------------------
-- Повтор чистки 0017 (регрессия 0018 могла успеть насоздавать по-сменных
-- записей) — идемпотентно, разобранные вручную не трогаются.
-- -----------------------------------------------------------------------------
