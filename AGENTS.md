...
- **Доступность полевых экранов** (телефон в перчатках, экранная читалка, клавиатура):
  - Цель нажатия на рабочих экранах — не меньше 44px: у `Button` для этого есть **проектный** размер `size="field"` (и `icon-field`). Он дописан в генерируемый `src/components/ui/button.tsx` вручную — после `shadcn add button` вернуть.
  - Подпись группы контролов (группа кнопок, кейпад, список карточек) — `GroupLabel` + `role="group" aria-labelledby` на контейнере. `<Label>` без `htmlFor` не оставлять: он ни с чем не связан. Для одиночного поля — обычный `<Label htmlFor>` + `id`.
  - Полноэкранные слои (подпись, сканер QR) — только через `FullscreenSheet` (`src/components/field/fullscreen-sheet.tsx`): свой `fixed inset-0` не даёт ни фокус-трапа, ни Escape, ни блокировки фона.
  - ⚠️ **Внутри `FullscreenSheet` инициализация «`ref` + `useEffect([])`» не работает.** Портал base-ui рендерит содержимое на пару коммитов позже: эффект отрабатывает, когда `ref.current` ещё `null`, молча выходит по проверке и больше не повторяется. Так три дня не работали подпись (`signature_pad` не создавался — окно открывается, а холст пустой) и сканер QR (`new Html5Qrcode(id)` не находил элемент, и `catch` показывал «камера недоступна»). Библиотеку, которой нужен живой DOM-узел, заводить **по факту монтирования узла**: ref-колбэк отмечает появление элемента, эффект перезапускается на нём (образец — `attachCanvas` в `src/components/field/signature-pad.tsx`).
  - Скрытый `<input type="file">` — `sr-only`, а не `hidden`: `display:none` убирает поле из фокуса. Исключение — input, который открывают программно по `ref.click()`.
  - Значения, меняющиеся без ввода (набранные литры, остаток, «сохранено»), заворачивать в `<output>` или `role="status"`, ошибки — `role="alert"`.
  - Главную кнопку экрана не гасить через `disabled`: серая кнопка не объясняет, чего не хватает. Проверять на клике и уводить фокус в первый незаполненный блок (образец — `firstMissing()` в `src/app/fleet/fuel/issue/issue-form.tsx`).
